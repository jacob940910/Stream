...
public class StreamMain {
	...
	public static void main(String[] args) {
		...
		*/
		/*
		//성별이 남자인 데이터의 평균 점수를 구하는데 소수 1째 자리에서 반올해서 
		//정수 부분만 출력
		...
		*/
		...
		List<StudentVO> result = stream.filter(data -> data.getGender().equals("남자") 
				&& data.getScore() >= 90).collect(Collectors.toList());
		for(StudentVO vo : result) {
			System.out.println(vo);
		}
		*/
		/*
		//groupingBy에 작성한 함수의 결과를 key로 하고 원래데이터의 List값으로 해서 Map으로 리턴합니다.
		Map<String,List<StudentVO>> map = stream.collect(Collectors.groupingBy(StudentVO:: getGender));
		System.out.println(map);
		*/
		
		/*
		Map<String,IntSummaryStatistics> sum = stream.collect(Collectors.groupingBy(StudentVO::getGender,
				Collectors.summarizingInt(StudentVO::getScore)));
		*/
		
		//gender 별로 score의 합계를 정수로 출력
		Map<String, Integer> map = stream.collect(Collectors.groupingBy(StudentVO::getGender,
