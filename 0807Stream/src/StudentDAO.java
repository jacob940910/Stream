...
public class StudentDAO {
	...
	//StudentDAO클래스에서 모든 데이터를 가져오는 메소드
	public List<StudentVO> fetch(){
		//리턴할 리스트를 생성
		//List를 리턴해야 하는 경우에는 null을 리턴하지 않는게 좋습니다.
		//List는 특별한 경우가 아니면 빠른 열거를 이용해서 접근 하는데 
		...
		try {
			...
			while(rs.next()) {
				...
				StudentVO vo = new StudentVO(rs.getString("num"),rs.getString("name"),rs.getString("subject"),rs.getInt("score"),
